...
- `backend/docs/`: RAG の知識ソース（`.md` / `.txt` / `.pdf`）。起動時に読み込まれます。PDF はテキスト抽出して取り込みます。
...
画面下部の「ナレッジ文書」セクションでは、登録済み文書の一覧表示・`.md`/`.txt`/`.pdf` のアップロード・削除ができます（アップロード分は再起動後も残ります）。
PDF は実機で確認済みです。`backend/docs/test-jp-rag-20260526.pdf` のような、文字を選択できる可読 PDF なら検索対象として取り込めます。
...
RAG の知識ソースを実行時に追加・確認・削除できます（`.md` / `.txt` / `.pdf`、最大 10MB）。PDF はテキストを抽出して取り込みます（テキストが取れない画像 PDF は 400）。実機では、文字列として選択できる PDF の取り込みと質問応答を確認済みです。
...
1. **起動時（ingest）**: `backend/docs/` の `.md` / `.txt` / `.pdf` を読み込み（PDF はテキスト抽出）、文字数ベースでチャンク分割し、埋め込みモデルでベクトル化して **pgvector の `chunks` テーブル**に保存します（埋め込み次元はモデルから自動検出し、HNSW インデックスを作成）。
