...
- **backend/** — NestJS 11. `AppController` exposes routes under the `api` prefix: `GET /api/message` (static placeholder) and `POST /api/chat` (`{ "message": string }` → `{ "message": string }`). CORS is enabled and the server binds `0.0.0.0:3000` (`main.ts`). The RAG layer lives in `backend/src/rag/` (`RagModule`), imported by `AppModule`.
...
- `EmbeddingService` calls the OpenAI-compatible `/v1/embeddings` (same `OPENAI_BASE_URL`); disabled when `OPENAI_EMBEDDING_MODEL` is empty. `VectorStoreService` stores chunks in **pgvector** (PostgreSQL `chunks` table, `embedding vector(N)` + HNSW cosine index) using raw `pg` (node-postgres), no ORM. `IngestService` (`OnModuleInit`) loads `docs/`, chunks (`chunk.util.ts`, char-based with overlap), embeds, then `reset()`s the table (drop/create at the model's embedding dimension, auto-detected) and inserts. `RagService.retrieve()` embeds the query, runs a cosine-distance (`<=>`) search, and logs hit sources + scores.
- The `chunks` table is rebuilt every startup, so it always matches the current `docs/` and embedding dimension (handles switching e.g. 768↔1024). Data lives in the `pgdata` volume. `VectorStoreService.size` is an in-memory count (set after ingest) used for the synchronous `RagService.isReady()` check.
