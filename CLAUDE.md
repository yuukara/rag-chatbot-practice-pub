...
- **backend/** — NestJS 11. `AppController`: `GET /api/message` (static placeholder) and `POST /api/chat` (`{ "message": string }` → `{ "message": string }`). `DocumentsController` (in `RagModule`): `GET /api/documents` (list sources), `POST /api/documents` (multipart `file`, `.md`/`.txt`/`.pdf`, ≤10MB, runtime ingest), `DELETE /api/documents/:source`. CORS is enabled and the server binds `0.0.0.0:3000` (`main.ts`). The RAG layer lives in `backend/src/rag/` (`RagModule`), imported by `AppModule`.
...
- **PDF support**: `pdf.util.ts` extracts text via `pdf-parse`, pinned to the bundled pdfjs `v2.0.550` (the default `v1.10.100` is too old and throws "bad XRef entry" on some PDFs). `.md`/`.txt` are read as UTF-8. Image-only PDFs (no extractable text) → 400. Upload limit 10MB, enforced at both multer and nginx (`client_max_body_size`). Verified with a real selectable-text PDF in `backend/docs/test-jp-rag-20260526.pdf`.
