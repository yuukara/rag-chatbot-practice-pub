...
## 文書アップロード機能（バックエンド・フロント UI とも実装済み）

ブラウザからファイルをアップロードして RAG の知識を増やせるようにする。
`docs/` 同梱・起動時 ingest に加え、実行時のアップロード取り込みに対応した。

### フロント UI 実装状況（2026-05-25 完了）

- `app.component.ts` に文書管理を追加: 読み込み時に `GET /api/documents` で一覧表示、`.md`/`.txt` のアップロード（`POST`、FormData）、source 単位の削除（`DELETE`）
- チャットパネル内に「ナレッジ文書」セクションを追加（一覧・チャンク数・削除・ファイル選択・アップロード状態）。既存の緑系テーマに合わせた最小追加
- ヘッドレス Chrome で描画を確認（一覧が API 取得で表示される）
- `angular.json` の anyComponentStyle 予算を 4kb/6kb に引き上げ（UI 追加でコンポーネント CSS が 2kb をわずかに超えたため）
